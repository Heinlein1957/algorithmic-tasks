function count_borders(r::Robot)::Int
    ...
    while !isborder(r, Nord)
        possible, is_border = move_if_possible!(r, side)
...
function move_if_possible!(r::Robot, direct_side::HorizonSide)::Tuple{Bool, Bool}
    ...
    if isborder(r,direct_side) == false
        move!(r,direct_side)
        if num_steps > 0
            if isborder(r,reverse_side) && !isborder(r,direct_side)
                isRectangular = true
            end
    ...
    return result, (num_steps == 1 && isRectangular) || (num_steps == 1 && result && !isRectangular)
end

function move_nord_and_back!(r::Robot)::Bool
    direct_side = Nord
    orthogonal_side = next(direct_side)
    reverse_side = inverse(orthogonal_side)
    num_steps=0
    ...
    return num_steps == 1 && result && !isRectangular
end
